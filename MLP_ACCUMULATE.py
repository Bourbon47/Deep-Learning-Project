...
rows = np.loadtxt("C:\Users\wontae\Desktop\Study_\School_class\3-2\통계학과딥러닝pbl", delimiter=",")
...
xs=np.zeros(((train_idx[1]-train_idx[0]),45))
ys=np.zeros((((train_idx[1]-train_idx[0]),45)))

xs_v=np.zeros(((val_idx[1]-val_idx[0]+1),45))
ys_v=np.zeros(((val_idx[1]-val_idx[0]+1),45))

xs_t=np.zeros(((test_idx[1]-test_idx[0]+1),45))
ys_t=np.zeros(((test_idx[1]-test_idx[0]+1),45))

xs_tt=np.zeros((len(x_samples),45))
ys_tt=np.zeros((len(x_samples),45))
xs_accu=np.zeros((len(x_samples),45))




for i in range(0, test_idx[1]):
    xs_tt[i] = x_samples[i].reshape(1, 45)
    ys_tt[i] = y_samples[i].reshape(1, 45)



for i in range(train_idx[0], train_idx[1]):
    xs[i] = x_samples[i].reshape(1, 45)
    ys[i] = y_samples[i].reshape(1, 45)

for i in range(val_idx[0]-1, val_idx[1]):
    xs_v[i-val_idx[0]+1] = x_samples[i].reshape(1, 45)
    ys_v[i-val_idx[0]+1] = y_samples[i].reshape(1, 45)

for i in range(test_idx[0]-1, test_idx[1]):
    xs_t[i-test_idx[0]+1] = x_samples[i].reshape(1, 45)
    ys_t[i-test_idx[0]+1] = y_samples[i].reshape(1, 45)
    
    
xs_accu_tr=np.zeros(((train_idx[1]-train_idx[0]),45))
xs_accu_val=np.zeros(((val_idx[1]-val_idx[0]+1),45))
xs_accu_test=np.zeros(((test_idx[1]-test_idx[0]+1),45))
xs_accu_tt=np.zeros((len(x_samples),45))

for i in range(0, test_idx[1]):
    for j in range(0,i):
        xs_accu_tt[i]=xs_accu_tt[i]+xs_tt[j] 


        
for i in range(train_idx[0], train_idx[1]):
    xs_accu_tr[i]= xs_accu_tt[i]

for i in range(val_idx[0]-1, val_idx[1]):
     xs_accu_val[i-val_idx[0]+1] = xs_accu_tt[i]
     
for i in range(test_idx[0]-1, test_idx[1]):
    xs_accu_test[i-test_idx[0]+1] = xs_accu_tt[i]

    
    
# 모델을 정의합니다.
model=Sequential()
model.add(Dense(100, activation='relu',input_shape = (45,)))
...
history= model.fit(xs_accu_tr,ys,epochs=100)
...
for i in range(len(x_samples)):
    ys_pred = model.predict(xs_tt[i].reshape(1,45)) # 모델의 출력값을 얻음
...
print('Total') 
print('===================================================================================================')    
print('Train(800회 X 10 = 8000번)  1등 : {0:5d} 2등 : {1:5d} 3등 : {2:5d} 4등 : {3:5d} 5등 : {4:5d} 6등 : {5:5d} 총상금 : {6:15,d}'.format(train_total_grade[0], train_total_grade[1], train_total_grade[2], train_total_grade[3], train_total_grade[4], train_total_grade[5], int(sum(train_total_reward))))
print('Val(100회 X 10 = 1000번)    1등 : {0:5d} 2등 : {1:5d} 3등 : {2:5d} 4등 : {3:5d} 5등 : {4:5d} 6등 : {5:5d} 총상금 : {6:15,d}'.format(val_total_grade[0], val_total_grade[1], val_total_grade[2], val_total_grade[3], val_total_grade[4], val_total_grade[5], int(sum(val_total_reward))))
print('Test(91회 X 10 = 910번)     1등 : {0:5d} 2등 : {1:5d} 3등 : {2:5d} 4등 : {3:5d} 5등 : {4:5d} 6등 : {5:5d} 총상금 : {6:15,d}'.format(test_total_grade[0], test_total_grade[1], test_total_grade[2], test_total_grade[3], test_total_grade[4], test_total_grade[5], int(sum(test_total_reward))))
print('===================================================================================================')
...
model1=Sequential()
model1.add(Dense(100, activation='tanh',input_shape = (45,)))
...
history= model1.fit(xs_tt,ys_tt,epochs=2)

pred_xs=(ys[len(x_samples)-1]+xs_accu_tt[len(x_samples)-1]).reshape(1,45)
predict_y_last=model1.predict(pred_xs)
